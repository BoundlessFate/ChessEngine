...
class ds_hashset {
	...
	public:
		...
		std::pair< iterator, bool > insert(KeyType const& key) {
			...
			while (itr != m_table[index].end()) {
				if (*itr == key)
					return std::make_pair(iterator(this, index, m_table[index].begin()), false);
				itr++;
			}
	...
	private:
		...
		for (unsigned int i=0; i<m_table.size(); i++) {
			for (hash_list_itr p=m_table[i].begin(); p != m_table[i].end(); p++) {
				unsigned int value = m_hash(*p);
				unsigned int index = value % new_table.size();
